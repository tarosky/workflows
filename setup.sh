...
if [ -f package.json ]; then
	npm install --save-dev @wordpress/env @wordpress/scripts @wordpress/stylelint-config @wordpress/create-block @kunoichi/grab-deps cssnano glob imagemin imaggemin-gifsicle imagemin-pngquant imagemin-svgo npm-watch postcss-cli
fi
